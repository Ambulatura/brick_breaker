...
void Collision::collision(Ball& ball, Bricks& bricks) {
	for (uint32_t i = 0; i < bricks.elements.size(); i++) {
		Brick& brick = bricks.elements[i];
		if (brick.life > 0) {
			auto collision_info = Collision::check_collision(ball, brick);
...
std::tuple<bool, Direction, glm::vec2> Collision::check_collision(const Ball& ball, const Brick& brick) {
